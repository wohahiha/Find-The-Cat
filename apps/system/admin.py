...
class AdminAuditMixin:
    ...
    def _get_field_changes(self, request, obj, form=None):
        """
        获取字段变更详情
...
class SystemConfigAdmin(AdminAuditMixin, admin.ModelAdmin):
    ...
    class SystemConfigAdminForm(forms.ModelForm):
        ...
        def __init__(self, *args, **kwargs):
            ...
            # 切换字段类型与初始值
            elif cfg.value_type == SystemConfig.ValueType.INT:
...
class SystemLogAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request: HttpRequest):
        ...
        try:
            ...
            if not Path(log_file_path).exists():
                ...
                # 读取最新 limit 条日志（默认倒序返回）
                log_entries = list(parser.parse_file(limit=limit, order="desc"))
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        current_limit = getattr(request, "_systemlog_limit", self.default_limit)
        ...
        extra_context["show_load_more"] = True
    ...
    def get_urls(self):
        ...
        custom_urls = [
            ...
            path("export/json/", self.admin_site.admin_view(self.export_logs_json), name="system_systemlog_export_json"),
        ]
        return custom_urls + urls

    def export_logs_txt(self, request: HttpRequest) -> HttpResponse:
        """
        导出日志文件 - TXT格式（原始日志文件）
    ...
    def export_logs_csv(self, request: HttpRequest) -> HttpResponse:
        """
        导出日志文件 - CSV格式
    ...
    def export_logs_json(self, request: HttpRequest) -> HttpResponse:
        """
        导出日志文件 - JSON格式
...
class SystemLogCategoryAdmin(admin.ModelAdmin):
    ...
    def category_name_link(self, obj):
        ...
        # 返回带链接的 HTML
        return format_html('<a href="{}">{}</a>', url, obj.category_name)
