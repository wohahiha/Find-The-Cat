...
class ConfigService(BaseService[SystemConfig]):
    ...
    SUPPORTED_CONFIGS = {
        ...
        "DEBUG": {
            ...
            "detail": "控制 Django 是否开启调试模式，影响错误栈展示和静态资源加载方式。生产环境必须关闭以避免泄露敏感信息，开启只用于本地调试。首次启动时默认为 True（开发环境），生产环境请在后台修改为 False。修改后需重启服务生效。",
            "required": True,
        },
        "ALLOWED_HOSTS": {
            "type": SystemConfig.ValueType.JSON,
            "desc": "允许的主机列表",
            "detail": "限制可访问站点的域名或 IP，防止 Host 头攻击。填写 JSON 数组格式，如 [\"example.com\", \"127.0.0.1\"]。首次启动时默认为 [\"*\"]（允许所有），生产环境必须在后台修改为实际域名/IP。修改后需重启服务生效。",
            "required": True,
        },
        "ALLOW_LOGIN_WITHOUT_CAPTCHA": {
            "type": SystemConfig.ValueType.BOOL,
            "desc": "允许登录跳过图形验证码（仅测试用）",
            "detail": "控制登录时是否强制校验图形验证码。仅在联调或自动化测试时可暂时置为 True，生产环境应保持 False。",
            "required": True,
        },
        ...
        "LOG_OUTPUT_LEVEL": {
            "type": SystemConfig.ValueType.STRING,
            "desc": "日志输出级别",
            "detail": "控制哪些级别的日志会被输出到文件。可选值：Low（仅错误，输出 ERROR、CRITICAL）、Medium（包含警告，输出 WARNING、ERROR、CRITICAL）、High（全部日志，输出 INFO、WARNING、ERROR、CRITICAL）。生产环境建议 Medium，问题排查时使用 High。",
            "required": True,
        },
        # Celery
