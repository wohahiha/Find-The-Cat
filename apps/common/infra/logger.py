...
- 从系统配置读取LOG_OUTPUT_LEVEL和LOG_PATH
- 支持Low/Medium/High三级输出控制
- 支持JSON和PLAIN两种格式
- 自动轮转日志文件（按日期）
...
def get_log_level_from_config() -> int:
    """
    从系统配置读取LOG_OUTPUT_LEVEL并映射到Python logging级别
    ...
    if not django_apps.apps_ready:
        return get_log_level_from_settings()
    try:
        from apps.system.models import SystemConfig

        config = SystemConfig.objects.filter(key="LOG_OUTPUT_LEVEL").first()
        if not config:
            return get_log_level_from_settings()

        return LOG_LEVEL_MAP.get(config.value, logging.WARNING)
    except Exception:
        # 如果数据库未初始化或配置不存在，返回默认值
        return get_log_level_from_settings()


def get_log_level_from_settings() -> int:
    """基于 settings.LOG_OUTPUT_LEVEL 计算日志级别"""
    level_name = getattr(django_settings, "LOG_OUTPUT_LEVEL", "Medium")
    return LOG_LEVEL_MAP.get(level_name, logging.WARNING)
...
def configure_logging(force: bool = False, *, level: Optional[int] = None, log_file_path: Optional[str] = None) -> None:
    ...
    从系统配置读取：
    - LOG_OUTPUT_LEVEL: 控制输出级别（Low/Medium/High）
    - LOG_PATH: 日志目录路径
    ...
    # 获取配置
    log_level = level if level is not None else get_log_level_from_config()
